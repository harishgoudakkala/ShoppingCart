...
function calculateEmi(principalPaise, annualRate, tenureMonths) {
  ...
  if (!Number.isFinite(principal) || !Number.isFinite(rate) || !Number.isInteger(months) || months <= 0) {
...
function serializePlan(row, principalPaise) {
  const monthlyPayment = calculateEmi(principalPaise, row.interestRate, row.tenure);
  const totalInstallments = monthlyPayment * Number(row.tenure);
...
const planSelect = `
  ...
  FROM emi_plans e
  JOIN financing_programs f ON f.id = e.financing_program_id
    ...
    const { rows } = await query(`
      SELECT
        ...
        (
          SELECT vi.image_url
          FROM variants vv
          JOIN variant_images vi ON vi.variant_id = vv.id
...
app.get(`${API_PREFIX}/products/:slug`, async (req, res, next) => {
  try {
    const productResult = await query(`
      SELECT id, slug, name, badge, description
      FROM products
      WHERE slug = $1
      LIMIT 1;
    `, [req.params.slug]);
    ...
    const variantResult = await query(`
      SELECT
        ...
        COALESCE(
          json_agg(
            json_build_object('id', vi.id, 'imageUrl', vi.image_url, 'sortOrder', vi.sort_order)
    ...
    const requestedVariantId = req.query.variantId === undefined ? null : Number(req.query.variantId);
    if (req.query.variantId !== undefined && !Number.isInteger(requestedVariantId)) {
      return res.status(400).json({ success: false, message: 'variantId must be an integer' });
    }

    const selectedVariant = requestedVariantId === null
      ? variants[0]
      : variants.find((variant) => variant.id === requestedVariantId);

    if (!selectedVariant) {
      return res.status(400).json({ success: false, message: 'Invalid variantId for this product' });
    }

    const emiResult = await query(planSelect, [selectedVariant.id]);
    const emiPlans = emiResult.rows.map((row) => serializePlan(row, selectedVariant.price));
...
app.get(`${API_PREFIX}/products/:slug/emi-plans`, async (req, res, next) => {
  try {
    ...
    const variantResult = await query(`
      SELECT v.id, v.price_paise AS price
      FROM variants v
      JOIN products p ON p.id = v.product_id
      WHERE v.id = $1 AND p.slug = $2
      LIMIT 1;
    ...
    res.json({
      success: true,
      data: rows.map((row) => serializePlan(row, variantResult.rows[0].price))
    });
  } catch (error) {
    next(error);
  }
...
app.listen(PORT, () => {
  console.log(`XYZ API running on http://localhost:${PORT}`);
});
